...
> Câu hỏi:
> - Em thấy dòng thông báo nào trong console? Trong console em thấy dòng thông báo "Hello from JavaScript!"
> - Điều này cho em biết JavaScript đang làm gì khi trang web được tải? Điều này cho thấy khi trang web được tải, trình duyệt đã chạy file main.js và chạy lệnh console.log() để in thông báo ra console.
...
> - CSS: Định dạng kiểu dáng, màu sắc và bố cục (giao diện).
> - JavaScript: Xử lý các hành động, logic và tương tác động trên trang web.
...
>   - `number` là: Kiểu dữ liệu số (số nguyên hoặc số thực)
>   - `string` là: Kiểu dữ liệu chuỗi văn bản (đặt trong dấu ngoặc kép hoặc đơn)
>   - `boolean` là: Kiểu dữ liệu logic, chỉ có hai giá trị là true hoặc false
...
> Câu hỏi:
> - Dòng log hiển thị gì sau khi em sửa thông tin? Dòng log hiển thị: Xin chào, mình là Quyết, năm nay mình 20 tuổi.
> - Nếu em quên dấu `;` hoặc quên dấu `+`, điều gì xảy ra? Trình duyệt báo lỗi thế nào?
    - Quên ;: Đa số trường hợp JS vẫn chạy được nhờ cơ chế tự động chèn dấu chấm phẩy.
    - Quên +: Trình duyệt sẽ báo lỗi Uncaught SyntaxError: Unexpected string vì sai cú pháp nối chuỗi

---

#### Bước 5: Phản tư nhanh (Reflection)

> - Điều thú vị nhất em vừa khám phá được về console là gì? Điều thú vị đó là console là một môi trường chạy thử Javascript trực tiếp trên web
> - Em gặp lỗi cú pháp nào? Em đã xử lý bằng cách nào (tự sửa, hỏi bạn, đọc lỗi, tìm Google, …)? em hay gặp lỗi cú pháp, khi gặp lỗi này thường kiểm tra lại bằng AI.
...
> - Khi nào chương trình in `"Giỏi"`? khi mà score lớn hơn hoặc bằng 8
> - Khi nào chương trình in `"Yếu"`? khi mà score nhỏ hơn 5
...
> - Ưu điểm của việc dùng hàm thay vì lặp lại cùng một đoạn code nhiều lần là gì? Ưu điểm là tránh lặp lại code, có thể tái sử dụng nhiều lần, code dễ bảo trì.
...
> - Phần nào trong if/else hoặc hàm khiến em khó hiểu nhất? không có, do đã làm quen ở trong môn nhập môn lập trình 
...
> Câu hỏi:
> - `document.getElementById("status")` đang làm gì? document.getElementById("status") đang lấy phần tử HTML có id "status" trong trang web.
> - Sự kiện `"click"` xảy ra khi nào? Xảy ra khi người dùng nhấn chuột (hoặc chạm tay) vào phần tử
> - Trong đoạn code trên, khi nhấn nút `btnHello`, điều gì thay đổi trên trang? Thì Nội dung của đoạn
sẽ thay đổi thành: Xin chào! Đây là nội dung được thay đổi bằng JavaScript.
...
> - Em hãy ghi lại 1 ví dụ khác mà JavaScript có thể làm với `document.body.style`. document.body.style.boxShadow = "0 0 20px rgba(0,0,0,0.5)";
...
> Câu hỏi:
> - Sự kiện `"input"` khác gì so với `"click"`? Input xảy ra khi khi gõ hoặc thay đổi text còn click xảy ra khi nhấn chuột
> - Khi em xoá hết nội dung ô input, dòng `greeting` hiển thị gì? sẽ hiển thị: Xin chào, ! 
...
>   DOM là cách mà trình duyệt biến trang HTML thành một cấu trúc dạng cây gồm nhiều đối tượng. Mỗi thẻ HTML được xem như một node (nút) trong cây. Nhờ vậy, JavaScript có thể truy cập, thay đổi nội dung, thuộc tính hoặc kiểu hiển thị của các phần tử trên trang web
